...
**Текущий payoff exception:** существующий `assets/art/characters/happy-toto-loop-sampled/frames/happy-toto-loop-animation.anim` сохранён как editor-created clip и перемещён вместе со своей рабочей нодой `HappyTotoAnimatedSprite` из EndCard в `ThreatLayer/CageRoot/CageSwingRoot`. Он включается через `playOnLoad` только при активации freed-node после `cage-open`; EndCard не использует AnimationClip для Тото.
