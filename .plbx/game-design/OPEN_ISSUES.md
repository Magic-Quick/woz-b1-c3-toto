...
| `OI-522` | Resolved | Краш превью `Cannot read properties of undefined (reading '0')` (поток) при переходе в финал. Стек: `Simple.updateUVs → Sprite.updateRenderer → Director.tick`. Причина: при переносе `toto-full-body.png` на `EndTotoRoot` Sprite `_spriteFrame` получил UUID `08ccdbe1-14dd-4f2e-9f0c-5d3be85734dd` БЕЗ `@f9941` суффикса и с `__expectedType__: "cc.ImageAsset"` вместо `"cc.SpriteFrame"`. Cocos резолвил UUID как главный ImageAsset (без `.uv`), а не как SpriteFrame sub-asset → `spriteFrame.uv` = `undefined` → `uv[0]` краш каждый кадр. Фикс: `set_asset_ref` с `08ccdbe1...@f9941` + `expectedType: cc.SpriteFrame`. Дополнительно: `HappyTotoAnimatedSprite.Animation.playOnLoad` установлен `false` + явный `Animation.play()` из `SaveTotoThreatView.playTotoFreedAnimation()` (рабочий паттерн, зеркалит endcard). |
