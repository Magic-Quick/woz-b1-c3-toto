...
- [ ] После первого spin-win (`Oz`) огонь снижается с `F4` до `F3` ещё до bonus-фазы.
...
- [ ] После первого spin-win огонь уже ослаблен на один шаг.
- [ ] Pick 1 снижает огонь `F3→F2`.
- [ ] Pick 2 снижает огонь `F2→F1`.
- [ ] Pick 3 полностью гасит огонь `F1→F0`.
- [ ] При снижении огонь сжимается и по X, и по Y.
- [ ] По Y огонь сжимается только сверху; нижняя кромка визуально не подпрыгивает.
- [ ] После третьего замка видны все 3 открытых замка до старта payoff-перехода.
- [ ] Затем клетка переходит в отдельный `cage-open` state без door-swing.
