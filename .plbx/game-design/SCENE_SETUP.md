...
| coin/glow/money FX | `SaveTotoCoinBurst.prefab`, `SaveTotoMoneyBurst.prefab`, `SaveTotoGlowPulse.prefab` | on demand |

Prefabs создаются в `assets/prefabs/save-toto/**`; `.meta` создаёт Cocos.


## 4.3. AnimationClip requirements

Visual motion should be bound through `.anim` clips from `assets/animations/save-toto/**`:

| Scene/Prefab | Clips |
|---|---|
| `SpinButton` | `spin_pulse.anim` |
| `Basket_01..06` | `basket_idle_pulse.anim`, `basket_selected.anim` |
| `LockLeft/Center/Right` | `lock_open_remove_left/center/right.anim` |
| `FireRoot` | `fire_level_3_to_2.anim`, `fire_level_2_to_1.anim`, `fire_level_1_to_0.anim` |
| `EndCardLayer` / `CtaButton` | `packshot_intro.anim`, `cta_intro.anim`, `cta_pulse.anim` |
| `CoinFxRoot` / `FloatingRewardRoot` | `money_dollar_pop.anim`, `money_gold_coins_pop.anim`, `money_gold_bricks_pop.anim` |

Scene builder may create `Animation` components and clip slots, but `.anim` and `.meta` must be produced by Cocos/editor workflow.
...
| `FxLayer` | `40` | Lock FX, rewards, money/coin sprites, glow |
...
6. Если SpriteFrame/AnimationClip UUID неизвестен, ссылка оставляется пустой и создаётся manual import/wiring task.
...
- Visual animations подключены как `.anim` clips из `assets/animations/save-toto/**` или оставлены explicit wiring tasks.
- Нет ручных `.meta`.
